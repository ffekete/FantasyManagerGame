...
public class HammerSwingAction implements Action {
    ...
    public HammerSwingAction(int x, int y, Texture texture, Actor actor) {
        this.x = x;
        this.y = y;
    ...
    public void update() {
        ...
        if((inc > 0 && angle < end) || (inc < 0 && angle > end))
            RendererToolsRegistry.INSTANCE.getSpriteBatch().draw(texture, x,y + 0.4f, originX, originY, 1, 1, Config.Engine.ACTOR_HEIGHT, Config.Engine.ACTOR_HEIGHT, angle, 0,0, texture.getWidth(),texture.getHeight(), direction == Direction.UP || direction == Direction.LEFT, false);
        else
            finished = true;
    }
