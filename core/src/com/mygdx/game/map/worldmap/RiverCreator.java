...
public class RiverCreator {
    ...
    public void connect(Map2D map, Point p1, Point p2) {
        ...
        while ((current.getX() != p2.getX() || current.getY() != p2.getY())) {
            ...
            alreadyBuilt.add(ObjectFactory.create(River.class, map, ObjectPlacement.FIXED.X(current.getX()).Y(current.getY())));
    ...
    private boolean adjacentRoad(Map2D map, int x, int y, Set<WorldObject> recentRoad) {
        for (int[] i : new int[][]{{-1, 0},{1, 0}, {0, -1}, {0, 1}}) {
            ...
            if (o != null && !recentRoad.contains(o) && (River.class.isAssignableFrom(o.getClass())))
                return true;
